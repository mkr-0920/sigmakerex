...
struct SIG
{
	...
	void Prepend(const SIG& siglet)
	{
		// 在 vector 的 *开头* 插入 siglet 的字节和掩码
		bytes.insert(bytes.begin(), siglet.bytes.begin(), siglet.bytes.end());
		mask.insert(mask.begin(), siglet.mask.begin(), siglet.mask.end());
	}
